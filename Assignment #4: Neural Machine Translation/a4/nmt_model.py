...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        x_pad = pack_padded_sequence(x, lengths=source_lengths)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(x_pad)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.squeeze(
            torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_hidden, dim=2)),
            dim=2
        )
        ### END YOUR CODE
